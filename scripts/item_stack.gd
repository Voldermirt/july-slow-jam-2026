...
@export var time_to_peak_seconds : float = 0.5 
@export var time_to_ground_seconds : float = 0.25 
...
@export var item_stack_scene: PackedScene 
...
func _ready() -> void:
	...
	if item_stack_scene:
		item_stack = item_stack_scene.instantiate() as Node2D
		add_child(item_stack)
		item_stack.position = Vector2(0, -16) 
	else:
		push_warning("ItemStackScene is missing from the Player inspector configuration!")
